...
from agent.custom.utils.Logger import Logger  # 改用合并后的 Logger

# 直接创建 Logger 实例，返回的就是可用的 logger
...
class AspectRatioChecker(TaskerEventSink):
    ...
    def on_tasker_task(
        ...
        width, height = self.get_controller_resolution(controller, ensure_screencap=True)
